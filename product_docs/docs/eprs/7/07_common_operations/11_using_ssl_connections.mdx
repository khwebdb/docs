...
- If you are using PostgreSQL, on the SSL-enabled Postgres database server:
  ...
  Make the following client/cert files available on the publication/subscription database using an SSL connection:
  - `postgresql.crt`
  - `postgresql.pk8`
  - `root.crt`
  ...
  ```
  $ cd /var/lib/edb/.postgresql/

  $ cp /var/lib/edb/11/data/server.crt postgresql.crt

  $ cp /var/lib/edb/11/data/root.crt .

  $ cp /var/lib/edb/11/data/server.key postgresql.key
